...
pub async fn get_kube_client(namespace: &str) -> Client {
    init_namespace(namespace)
        .await
        .expect("init_namespace error")
}

pub async fn init_namespace(namespace: &str) -> Result<Client> {
...
pub async fn check_namespace(client: &Client, namespace_str: &str) -> Result<()> {
